...
(define (union S1 S2)
  ...
  (if (null? S1)
      S2
      (if (member (car S1) S2)
          (append (union (cdr S1) S2))
          (append (list (car S1)) (union (cdr S1) S2))))
)
...
(define (set-equal? S1 S2)
  "implement"
  (if (and (subset? S2 S1) (subset? S1 S2))
      #t
      #f)
)
...
(define (difference S1 S2)
  ...
  (if (null? S1)
      ...
      (if (not (member (car S1) S2))
          (append (list (car S1)) (difference (cdr S1) S2))
