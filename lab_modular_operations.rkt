...
#|-------------------------------------------------------------------------------
 ...
 | Pledge: I pledge my honor that I have abided by the Stevens Honors Ssystem
 |-------------------------------------------------------------------------------|#
...
(define (pulverizer a b)
  ...
  (cond
     ((= b 0) (list 1 0))
     ((not (= b 0))
      (define s1 (car (pulverizer b (modulo a b))))
      (define t1 (car (cdr (pulverizer b (modulo a b)))))
      (define s t1)
      (define t (- s1 (* (quotient a b) t1)))
      (list s t)
      )
    )
)
...
(define (mod-inverse a m)
  "implement"
  (cond
    ((not (= 1 (gcd a m))) (error-inverse))
    ((= 1 (gcd a m)) (modulo (car (pulverizer a m)) m))
    )
)