...
;; 5 PTS
(define (waterjug-helper cA cB g fA fB)
  (if (= fB g)
      (list (list fA fB))
      (if (= fB cB)
          (append (list(list fA fB)) (waterjug-helper cA cB g fA 0))
          (if (= fA 0)
              (append (list (list fA fB)) (waterjug-helper cA cB g cA fB))
              (if (> (+ fA fB) cB)
                  (append (list (list fA fB)) (waterjug-helper cA cB g (- fA (- cB fB)) cB))
                  (append (list (list fA fB)) (waterjug-helper cA cB g 0 (+ fA fB)))))))
)

(define (waterjug cA cB g)
  "implement"
  (if (wj-solvable? cA cB g)
      (waterjug-helper cA cB g 0 0)
      (list))
)
...
;; 5 PTS
(define (factorize-helper p n)
  (if (divides? p n)
      p
      (factorize-helper (+ p 1) n))
)
(define (factorizen>1 n)
  (if (= n 1)
      (list)
      (append (list (factorize-helper 2 n)) (factorizen>1 (/ n (factorize-helper 2 n)))))
)
