...
(define (mod-exp b e m)
  ...
  (cond
    ((= 0 e) (modulo 1 m))
    ((and (> e 0) (= 0 (modulo e 2)))
    (define x (mod-exp b (/ e 2) m))
    (define y (expt (modulo x m) 2))
    (modulo y m))
    ((and (> e 0) (= 1 (modulo e 2)))
    (define x (mod-exp b (- e 1) m))
    (define y (* (modulo x m) b))
    (modulo y m)))
)
...
;; 6 PTS
(define (encrypt-helper plaintext key)
  (if (null? plaintext)
      (list)
      (append (list (mod-exp (char->integer (car plaintext)) (car (cdr key)) (car key))) (encrypt-helper (cdr plaintext) key)))
)
...
;; 6 PTS
(define (decrypt-helper ciphertext key)
  (if (null? ciphertext)
      (list)
      (append (list (integer->char (mod-exp (car ciphertext) (car (cdr key)) (car key)))) (decrypt-helper (cdr ciphertext) key)))
)
