...
(define (rotate c)
  ...
  (define x (car c))
  (define y (car(cdr c)))
  (define negativeX (* -1 x))
  (define negativeY (* -1 y))
  (list (list x y) (list y negativeX) (list negativeX negativeY) (list negativeY x))
)
...
(define (combine-courses s1 s2)
  "implement"
  (define s1courses (car(cdr (cdr (cdr (cdr (cdr s1)))))))
  (define s2courses (car(cdr (cdr (cdr (cdr (cdr s2)))))))
  (append s1courses s2courses)
)
